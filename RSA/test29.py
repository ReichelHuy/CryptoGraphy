...
def solve(msg, idx):
    digest = int.from_bytes(emsa_pkcs1_v15.encode((msg+suffix).encode(), 768 // 8), 'big')
    e = gmpy2.powmod(s, -1, n) * digest % n
    io.sendline(dumps({'option':'claim', 'msg': msg + suffix, 'e':hex(e), 'index':int(idx)}).encode())
    response = loads(io.recvline())
    if 'secret' in response:
        return response['secret']
    else:
        print("Error: 'secret' key not found in response!")
        return None
